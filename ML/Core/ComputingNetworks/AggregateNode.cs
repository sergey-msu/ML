...
{
  /// <summary>
  /// Less-generic shortcut version of AggregateNode<TIn, TOut, TSubNode>
  /// </summary>
  /// <typeparam name="TIn">Input object type</typeparam>
  /// <typeparam name="TOut">Output object type</typeparam>
  public class AggregateNode<TIn, TOut> : AggregateNode<TIn, TOut, ComputingNode<TIn, TOut>>
  {
  }

  /// <summary>
  /// Represents aggregate computing node -
  /// a node that composed from another nodes, calculates the results (may be in parallel) and merges(aggregates) its outputs
  ///
  ///       TIn -> TOut
  ///     /             \
    ...
    where TSubNode : ComputingNode<TIn, TOut>
  {
    ...
    //  }
    //
    //  return m_MergeNode.TryGetSubnodeByParamIndex(idx, out result, out subidx);
    //}
